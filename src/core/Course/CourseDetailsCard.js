        ...
        <div className={`flex flex-col gap-4 rounded-md bg-richblack-700 p-4 text-richblack-5`} >
          ...
          <div className="px-4">
            ...
            <div className="flex flex-col gap-4">
              <button className="bg-yellow-50 rounded-lg text-black"  onClick = {user && course?.studentsEnrolled.includes(user?._id) ? () => navigate("/dashboard/enrolled-courses") : handleBuyCourse} >
                {user && course?.studentsEnrolled.includes(user?._id) ? "Go To Course" : "Buy Now"}
              </button>
              {(!user || !course?.studentsEnrolled.includes(user?._id)) && (
                <button onClick={handleAddToCart} className="bg-caribbeangreen-100 rounded-lg text-black"> Add to Cart </button>
              )}
            </div>
            <div>
