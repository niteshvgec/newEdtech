...
import { fetchCourseCategories } from "../../../../services/courseDetailsAPI";
...
// import { AddressFamily } from "axios";
...
export default function CourseInformationForm(){
  ...
  const onSubmit = async (data) => {                      //handle next button click
    if(editCourse) {
      if(isFormUpdated()) {
        const currentValues = getValues()
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 rounded-md border-[1px] border-richblack-700 bg-richblack-800 p-6" >
      ...
      <div className="flex flex-col space-y-2">
        ...
        <input  id="courseTitle" placeholder="Enter Course Title" {...register("courseTitle", { required: true })} className="form-style w-full  text-white h-[50px] border-b-2 border-white rounded-md  bg-richblack-700" />
      ...
      <div className="flex flex-col space-y-2">
        ...
        <textarea id="courseShortDesc" placeholder="Enter Description"  {...register("courseShortDesc", { required: true })} className="form-style resize-x-none min-h-[130px] w-full text-white bg-richblack-700" />
      ...
      <div className="flex flex-col space-y-2">
        ...
        <div className="relative">
          <input id="coursePrice"  placeholder="Enter Course Price" className="form-style w-full !pl-12 text-white h-[50px] border-b-2 border-white rounded-md  bg-richblack-700"
      ...
      <div className="flex flex-col space-y-2">
        ...
        <select  {...register("courseCategory", { required: true })} defaultValue="" id="courseCategory" className="form-style w-full text-white h-[50px] border-b-2 border-white rounded-md  bg-richblack-700" >
      ...
      <div className="flex flex-col space-y-2">
        <label className="text-sm text-richblack-5" htmlFor="courseBenefits" c>
          Benefits of the course <sup className="text-pink-200">*</sup>
        </label>
       
        <textarea  id="courseBenefits"  placeholder="Enter benefits of the course"  {...register("courseBenefits", { required: true })} className="form-style resize-x-none min-h-[130px] w-full bg-richblack-700 text-white"  />
